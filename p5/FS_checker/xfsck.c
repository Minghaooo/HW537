...
int  walk_inode(struct dinode *dip){
    ...
    for (int i =0;i<NDIRECT;i++){
        if(dip->addrs[i] != 0){
           if((dip->addrs[i] >994)||(dip->addrs[i]<28)){
    ...
    uint *indirect = (uint *)(img_ptr + (dip->addrs[12]) *BSIZE );
   // printf("indirect %d\n", dip->addrs[12]);
    for (int i =0; i<NINDIRECT;i++){

        if (*indirect != 0){
            if((*indirect > 994) || (*indirect < 28)){
...
void check_SuperBlock(){
    ...
    Dir_check(dip, 1);
    bitmap[29]=1;
    for ( int i =2; i<IPB*25;i++){
        total_block+=walk_inode(dip +i);
         Dir_check(dip,i);
    //   printf("the current block num is %d\n", total_block+28);
    }
    if(total_block+28> sb->nblocks){
        error("ERROR: superblock is corrupted");
    }
...
// record the usage
    ...
    for (int i = 0; i < ninodes; ++i)
    {
        if (dip[i].type > 0 && ck9_inodes_used[i] == 0)
        { // check 9
        ...
        if (dip[i].type == 2)
        {
            if (dip[i].nlink != ck9_inodes_used[i])
            { // check 11
